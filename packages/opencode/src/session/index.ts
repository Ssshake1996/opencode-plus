...
import type { ArchivedTopicData } from "./context-compressor"
...
export namespace Session {
  ...
  export function fromRow(row: SessionRow): Info {
    ...
    return {
      ...
      archive_topics: row.archive_topics ?? undefined,
      global_summary: row.global_summary ?? undefined,
      time: {
  ...
  export function toRow(info: Info) {
    return {
      ...
      archive_topics: info.archive_topics ?? null,
      global_summary: info.global_summary ?? null,
  ...
  export const Info = z
    .object({
      ...
      archive_topics: z.array(z.any()).optional(),
      global_summary: z.string().optional(),
    })
    .meta({
      ref: "Session",
    })
  export type Info = z.output<typeof Info> & {
    archive_topics?: import("./context-compressor").ArchivedTopicData[]
  ...
  export interface Interface {
    ...
    readonly setArchiveTopics: (input: {
      sessionID: SessionID
      archiveTopics: ArchivedTopicData[]
    }) => Effect.Effect<void>
    readonly setGlobalSummary: (input: {
      sessionID: SessionID
  ...
  export const layer: Layer.Layer<Service, never, Bus.Service | Config.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const setArchiveTopics = Effect.fn("Session.setArchiveTopics")(function* (input: {
        sessionID: SessionID
        archiveTopics: ArchivedTopicData[]
      }) {
        yield* patch(input.sessionID, { archive_topics: input.archiveTopics, time: { updated: Date.now() } })
      })

      const setGlobalSummary = Effect.fn("Session.setGlobalSummary")(function* (input: {
        sessionID: SessionID
        globalSummary: string
      }) {
        yield* patch(input.sessionID, { global_summary: input.globalSummary, time: { updated: Date.now() } })
      })
  ...
  export const setArchiveTopics = fn(
    z.object({ sessionID: SessionID.zod, archiveTopics: z.array(z.any()) }),
    (input) => runPromise((svc) => svc.setArchiveTopics({ sessionID: input.sessionID, archiveTopics: input.archiveTopics })),
  )

  export const setGlobalSummary = fn(
    z.object({ sessionID: SessionID.zod, globalSummary: z.string() }),
    (input) => runPromise((svc) => svc.setGlobalSummary({ sessionID: input.sessionID, globalSummary: input.globalSummary })),
  )

  export const getContextForPrompt = fn(
    z.object({
      sessionID: SessionID.zod,
