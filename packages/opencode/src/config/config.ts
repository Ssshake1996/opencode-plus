    ...
    .object({
      ...
      compaction: z
        .object({
          ...
          // Three-dimensional compression options (MulAgent-style)
          // Context budget
          context_max_chars: z
            .number()
            .int()
            .min(0)
            .default(0)
            .describe("Character budget for assembled context. 0 = auto (max_tokens * 0.5 * 4)"),

          // Compression level thresholds
          level_full: z.number().min(0).max(1).default(0.7).describe("Relevance threshold for full compression (≥this value)"),
          level_summary: z.number().min(0).max(1).default(0.3).describe("Relevance threshold for summary compression"),
          level_title: z.number().min(0).max(1).default(0.1).describe("Relevance threshold for title-only compression"),

          // Relevance weights
          weight_keyword: z.number().min(0).max(1).default(0.5).describe("Weight for keyword overlap (Jaccard similarity)"),
          weight_recall: z.number().min(0).max(1).default(0.3).describe("Weight for recall intent detection"),
          weight_decay: z.number().min(0).max(1).default(0.2).describe("Weight for time decay"),

          // Topic archiving
          archive_threshold: z.number().int().min(1).default(30).describe("Auto-archive threshold: turns count to trigger archiving"),
          archive_manual_threshold: z.number().int().min(1).default(6).describe("Manual compress threshold via /compress command"),
          decay_half_life_hours: z.number().min(1).default(24).describe("Time decay half-life in hours"),
          max_gap_turns: z.number().int().min(1).default(6).describe("Max turns per topic before splitting"),
          max_kw: z.number().int().min(1).default(8).describe("Max keywords to extract per topic"),
          hard_cap_turns: z.number().int().min(1).default(50).describe("Hard cap on total turns retained"),
        })
        .optional(),
      experimental: z
        .object({
